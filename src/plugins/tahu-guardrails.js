...
class TahuGuardrailsPlugin {
  constructor() {
    ...
    this.description =
      'AI Safety Guardrails Plugin for content validation and filtering';
    this.version = '1.0.0';
    this.guards = new Map();
    this.logger = null;
  ...
  async install(tahu) {
    ...
    this._debugLog('🛡️ Installing TahuGuardrails plugin...');
    ...
    try {
      ...
      this._debugLog('✅ TahuGuardrails plugin installed successfully!');
      return true;
    } catch (error) {
      this._debugError('❌ Failed to install TahuGuardrails plugin:', error);
      return false;
    }
  }
  ...
  _registerGuardrailsTools(tahu) {
    ...
    tahu.registerTool('validateContent', {
      ...
      parameters: {
        ...
        guardNames: {
          type: 'array',
          description: 'Specific guards to apply',
          default: [],
        },
        strictMode: {
          type: 'boolean',
    ...
    tahu.registerTool('createGuard', {
      ...
      parameters: {
        ...
        action: {
          type: 'string',
          description: 'Action to take: block, warn, sanitize',
          default: 'warn',
        },
      },
      execute: async (params) => {
  ...
  async validateContent(content, guardNames = [], strictMode = false) {
    ...
    for (const guardName of guardsToApply) {
      ...
      if (guardResult.violations.length > 0) {
        ...
        if (
          guard.action === 'block' ||
          (strictMode && guard.severity === 'critical')
        ) {
        ...
        for (const pattern of rule.patterns) {
          ...
          if (matches) {
            ...
            if (guard.action === 'sanitize' || rule.type === 'pii_protection') {
              result.sanitizedContent = this._sanitizeContent(
  ...
  removeGuard(guardName) {
    ...
    return {
      success: removed,
      message: removed
        ? `Guard '${guardName}' removed successfully`
  ...
  getViolationStats() {
    ...
    for (const [guardName, guard] of this.guards) {
      ...
      if (
        guard.severity &&
        stats.severityBreakdown[guard.severity] !== undefined
      ) {
  ...
  getInfo() {
    return {
      ...
      totalGuards: this.guards.size,
      availableTools: ['validateContent', 'sanitizeContent', 'createGuard'],
      defaultGuards: ['content_safety', 'pii_protection', 'prompt_injection'],
    };
  }
}
