...
export default function tahuCurrencyPlugin(tahuInstance) {
  tahuInstance.registerTool('convertCurrency', {
    ...
    description:
      'Convert an amount from one currency to another. Input: "amount fromCurrency toCurrency" (e.g., "100 USD to EUR").',
    execute: async (input) => {
      const match = input.match(
        /(\d+(\.\d+)?)\s*([A-Za-z]{3})\s*to\s*([A-Za-z]{3})/
      );
      if (!match) {
      ...
      const rates = {
        USD: { EUR: 0.92, IDR: 16200, JPY: 156.0 },
        EUR: { USD: 1.08, IDR: 17600, JPY: 170.0 },
        IDR: { USD: 0.000062, EUR: 0.000057, JPY: 0.0096 },
        JPY: { USD: 0.0064, EUR: 0.0059, IDR: 104.0 },
      };
  ...
  console.log(
    '🔌 Tahu-Currency plugin loaded: convertCurrency tool registered.'
  );
}
