  ...
  execute: async (input, vectorStoreManager) => {
    ...
    try {
      if (sourceType === 'text') {
        ...
        console.log(
          chalk.blue(`🌐 Fetching content from URL: ${sourceData}...`)
        );
        ...
        textToTrain = textToTrain.replace(
      ...
      console.log(
        chalk.blue(
          `📚 Training knowledge for '${knowledgeBaseName}' using ${storeType} store...`
        )
      );
      await vectorStoreManager.addDocument(
        knowledgeBaseName,
        textToTrain,
        storeType
      );
