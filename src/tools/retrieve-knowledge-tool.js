...
export const retrieveKnowledgeTool = {
  ...
  description:
    'Retrieves relevant information from a specified knowledge base. Input format: "knowledgeBaseName|storeType|query_text|k". Supported store types: sqlite, chromadb. k is optional (default 3). Example: "my_docs|sqlite|What are TahuJS features?|2"',
  execute: async (input, vectorStoreManager) => {
    ...
    console.log(
      chalk.blue(
        `📚 Retrieving knowledge from '${knowledgeBaseName}' using ${storeType} store for query: "${queryText}" (k=${k})...`
      )
    );
    try {
      const results = await vectorStoreManager.retrieveDocuments(
        knowledgeBaseName,
        queryText,
        storeType,
        {},
      ...
      if (
        ...
        error.message.includes(
          'Supabase vector store requires additional setup'
        )
      ) {
