...
export const ocrAdvancedTool = {
  name: 'ocr_advanced',
  description: 'Performs a flexible, multi-stage OCR and analysis on a local image file. Can optionally use an AI to enhance and format the raw OCR text. Language and custom model path for OCR are configurable.',
  execute: async (filePath, options = {}, llmManager) => {
    ...
    if (debug) {
      console.log(chalk.blue(`[DEBUG] OCR Advanced Tool started for: ${filePath}`));
    ...
    try {
      if (debug) console.log(chalk.yellow(`[DEBUG] Stage 1: Performing Raw OCR using language '${language}'...`));
      if (debug && langPath) console.log(chalk.yellow(`[DEBUG] Using custom model path: ${langPath}`));
      ...
      if (['.png', '.jpg', '.jpeg', '.bmp', '.gif'].includes(fileExtension)) {
        ...
        try {
          ...
          if (Array.isArray(rgbColors)) {
            ...
            if(debug) console.warn(chalk.yellow(`[DEBUG] Could not detect colors for ${filePath}. Error: ${colorError.message}`));
            colors = []; // Default to empty array on error
        }

      } else if (fileExtension === '.pdf') {
        throw new Error(`PDF files are not directly supported for raw OCR by this tool. Please convert PDF pages to images first, or ensure a PDF-to-image conversion library (e.g., 'pdf-poppler' or 'pdf2pic') is integrated if you intend to use this tool for PDFs.`);
      } else {
        throw new Error(`Unsupported file type for OCR: ${fileExtension}. This tool only supports image and PDF (with external conversion) files.`);
      }

      rawOcrResult = { metadata, fullText, confidence, words, colors };
      if (debug) console.log(chalk.green('[DEBUG] Stage 1: Raw OCR successful.'));

    } catch (error) {
      if (debug) console.error(chalk.red(`❌ [DEBUG] Error during Raw OCR (Stage 1): ${error.message}`), error);
    ...
    if (aiEnhanced) {
      if (debug) console.log(chalk.yellow('[DEBUG] Stage 2: Performing AI Enhancement...'));
      if (!llmManager) {
        throw new Error('AI enhancement requires the LLMManager to be configured and passed to the tool.');
      }
      if (!rawOcrResult.fullText || rawOcrResult.fullText.trim() === '') {
        if (debug) console.warn(chalk.yellow('[DEBUG] Skipping AI enhancement because raw OCR text is empty.'));
        ...
        try {
          const aiResponse = await llmManager.chat(fullPrompt, { responseFormat: 'text' });
          aiEnhancedResult = aiResponse.response; // Assuming .response holds the text
          if (debug) console.log(chalk.green('[DEBUG] Stage 2: AI Enhancement successful.'));
        } catch (error) {
          if (debug) console.error(chalk.red(`❌ [DEBUG] Error during AI Enhancement (Stage 2): ${error.message}`), error);
          throw new Error(`Failed during AI enhancement stage: ${error.message}`);
        }
      }
    }
    ...
    if (!outputFields || !Array.isArray(outputFields)) {
      if (debug) console.log(chalk.blue('[DEBUG] No outputFields specified, returning all data.'));
      return allData;
    }
    ...
    if (debug) console.log(chalk.green.bold('[DEBUG] OCR Advanced process completed successfully.'));
    return finalResult;
  },
};
