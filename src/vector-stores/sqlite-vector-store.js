...
export class SQLiteVectorStore {
    ...
    async addDocument(text, embedding) {
        ...
        const stmt = this.db.prepare('INSERT INTO knowledge_base (text, embedding) VALUES (?, ?)');
    ...
    async retrieveDocuments(queryEmbedding, k = 5) {
        ...
        const rows = this.db.prepare('SELECT text, embedding FROM knowledge_base ORDER BY timestamp DESC LIMIT ?').all(k);
        console.log(chalk.blue(`🔍 Retrieving ${rows.length} documents from SQLite knowledge base.`));
        return rows.map(row => ({
            text: row.text,
