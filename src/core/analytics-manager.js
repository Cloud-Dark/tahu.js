...
export class AnalyticsManager {
    ...
    _getCostPerThousandTokens(model, provider) {
        ...
        switch (provider) {
            case 'openrouter':
                if (model.includes('claude-3-opus')) return 75.00; // Example: $75 per 1M tokens (input)
                if (model.includes('claude-3-sonnet')) return 3.00; // Example: $3 per 1M tokens (input)
                if (model.includes('gpt-4')) return 30.00; // Example: $30 per 1M tokens (input)
                if (model.includes('gpt-3.5-turbo')) return 0.50; // Example: $0.50 per 1M tokens (input)
                if (model.includes('gemini-pro')) return 0.125; // Example: $0.125 per 1M tokens (input)
                return 1.00; // Default for unknown OpenRouter models
            case 'openai':
                if (model.includes('gpt-4')) return 30.00;
                if (model.includes('gpt-3.5-turbo')) return 0.50;
                return 1.00;
            case 'gemini':
                return 0.125; // Gemini Pro pricing example
            case 'ollama':
                return 0.00; // Typically free for local models
            default:
                return 0.00;
        }
    }
    ...
    recordCompletion(tokens, duration, model, provider) {
        ...
        this.requestCount++;
        console.log(chalk.gray(`📊 Analytics: Tokens: ${tokens}, Time: ${duration}ms, Cost: $${((tokens / 1000) * costPerThousand).toFixed(4)}`));
    }
