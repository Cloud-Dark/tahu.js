...
export class WorkflowManager {
  ...
  createWorkflow(workflowDefinition) {
    ...
    return {
      ...
      async execute(initialInput) {
        console.log(
          chalk.magenta(
            `🚀 Starting workflow with initial input: "${initialInput}"`
          )
        );
        const results = {};
        ...
        while (taskQueue.length > 0) {
          ...
          console.log(
            chalk.cyan(
              `  Executing task "${taskName}" by agent "${agentName}"...`
            )
          );
          ...
          try {
            const agentResult = await self.agentManager.runAgent(
              agentName,
              inputForAgent
            );
            results[taskName] = agentResult.response;
            console.log(
              chalk.green(`  ✅ Task "${taskName}" completed. Result stored.`)
            );
          } catch (error) {
            console.error(
              chalk.red(
                `  ❌ Task "${taskName}" failed for agent "${agentName}": ${error.message}`
              )
            );
            results[taskName] = `Error: ${error.message}`;
            throw new Error(
              `Workflow failed at task "${taskName}": ${error.message}`
            );
          }
        }
  ...
  async parallel(tasks) {
    console.log(
      chalk.blue(`⚡ Executing ${tasks.length} tasks in parallel...`)
    );
    const promises = tasks.map((task) => {
      ...
      return Promise.reject(
        new Error('Invalid task format for parallel execution.')
      );
    });
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ One or more parallel tasks failed: ${error.message}`)
      );
      throw error;
    }
  }
  ...
  async batch(promptsAndOptions) {
    console.log(
      chalk.blue(
        `📦 Processing ${promptsAndOptions.length} prompts in batch...`
      )
    );
    const promises = promptsAndOptions.map((item) => {
      if (!item.prompt) {
        return Promise.reject(
          new Error('Each batch item must have a "prompt" property.')
        );
      }
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ One or more batch prompts failed: ${error.message}`)
      );
      throw error;
    }
  }
