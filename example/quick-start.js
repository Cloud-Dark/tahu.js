...
async function quickStart() {
  ...
  if (!SUPABASE_URL.includes('YOUR_SUPABASE_URL') || !SUPABASE_ANON_KEY.includes('YOUR_SUPABASE_ANON_KEY')) {
      console.warn('⚠️  Supabase URL or Anon Key not set. Supabase RAG demo might fail.');
  }
  ...
  try {
    ...
    const simpleAgent = tahu.builder()
      ...
      .addCapabilities(tools.calculateTool.name, tools.trainKnowledgeTool.name, tools.retrieveKnowledgeTool.name) // Add RAG capabilities
    ...
    const trainTextResult = await tahu.runAgent('SimpleAssistant', 'train knowledge_base_rag|sqlite|text|TahuJS adalah framework AI yang kuat untuk Node.js.');
    ...
    const trainFileResult = await tahu.runAgent('SimpleAssistant', 'train knowledge_base_rag|sqlite|file|knowledge.txt');
    console.log('Train from file:', trainFileResult.response);

    // Train from a URL (example: TahuJS documentation page)
    const trainUrlResult = await tahu.runAgent('SimpleAssistant', 'train knowledge_base_rag|sqlite|url|https://github.com/Cloud-Dark/tahujs');
    console.log('Train from URL:', trainUrlResult.response);

    // Train knowledge to Supabase (requires Supabase setup)
    const trainSupabaseResult = await tahu.runAgent('SimpleAssistant', 'train customer_feedback|supabase|text|Pelanggan menyukai TahuJS karena mudah digunakan.');
    ...
    const founderQuestion = await tahu.runAgent('SimpleAssistant', 'siapa pendiri apipedia?');
    ...
    const tahuFeaturesQuestion = await tahu.runAgent('SimpleAssistant', 'Apa saja fitur TahuJS?');
    console.log('\nQuestion: Apa saja fitur TahuJS?');
    console.log('Response:', tahuFeaturesQuestion.response);

    // Question based on URL training
    const githubInfoQuestion = await tahu.runAgent('SimpleAssistant', 'Apa itu TahuJS menurut GitHub repo nya?');
    ...
    const customerFeedbackQuestion = await tahu.runAgent('SimpleAssistant', 'Apa yang disukai pelanggan tentang TahuJS?');
