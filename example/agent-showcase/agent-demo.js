...
async function runAgentDemo() {
  ...
  try {
    ...
    console.log(`- ${supportAgent.name} (${supportAgent.capabilities.join(', ')})`);
    console.log(`- ${translatorAgent.name} (${translatorAgent.capabilities.join(', ')})`);
    ...
    const codeTask = "Write a simple JavaScript function that calculates the factorial of a number recursively";
    ...
    const nlpTask = "Analyze the sentiment and extract key entities from this customer review: 'I absolutely love the new iPhone 15 Pro! The camera quality is amazing and the battery life has improved significantly. Apple really outdid themselves this time.'";
    ...
    const supportTask = "A customer is complaining that their order hasn't arrived after 2 weeks and they're very upset. They want a refund immediately. How should I handle this situation?";
    ...
    const translationTask = "Translate this business email to Spanish and French: 'Dear valued customer, thank you for your interest in our products. We would be happy to schedule a meeting to discuss your requirements.'";
    ...
    const customAgent = tahu.builder()
      .name('AIConsultant')
      .systemPrompt('You are an AI consultant specializing in helping businesses implement AI solutions. You provide practical advice, technical guidance, and strategic recommendations.')
      .addCapabilities('chat', 'search', 'calculate', 'analyzeSentiment', 'classifyText')
      .addPersonality(['professional', 'knowledgeable', 'strategic'], 'confident', ['ai', 'business', 'technology'])
      .addMemory('volatile', { maxMemorySize: 30 })
      .build();

    console.log('Custom Agent Created:');
    console.log(`Name: ${customAgent.name}`);
    console.log(`System Prompt: ${customAgent.systemPrompt.substring(0, 100)}...`);
    console.log(`Capabilities: ${customAgent.capabilities.join(', ')}`);
    console.log(`Memory Type: ${customAgent.memoryType}`);

    // Test the custom agent
    const consultantTask = "A small retail business wants to implement AI to improve customer experience and increase sales. They have a limited budget of $10,000. What AI solutions would you recommend?";
    const consultantResult = await tahu.runAgent('AIConsultant', consultantTask);
    ...
    const feedback = "The new app is pretty good but the loading times are terrible. The user interface is confusing and I can't find basic features easily. Customer support was helpful though.";
    
    console.log('\n1. TextAnalyzer analyzing customer feedback...');
    const feedbackAnalysis = await tahu.runAgent('TextAnalyzer', 
    ...
    const responseStrategy = await tahu.runAgent('HelpDesk',
    ...
    const technicalSolution = await tahu.runAgent('CodeMaster',
