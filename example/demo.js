...
async function comprehensiveDemo() {
  ...
  console.log(
    '🔌 Attempting to automatically discover plugins from ./src/plugins...'
  );
  ...
  try {
    ...
    const currencyConversion = await tahu.useTool(
      'convertCurrency',
      '100 USD to IDR'
    );
    ...
    const travelResult = await tahu.runAgent(
      'TravelExpert',
      'Plan a day trip to Jakarta. Find interesting places to visit and provide directions.'
    );
    ...
    const researchResult = await tahu.runAgent(
      'MySmartResearcherJSON',
      'Research the current state of AI development in Indonesia'
    );
    console.log('Researcher Agent (JSON):', researchResult.response);
    // Run again to demonstrate memory persistence and trimming
    const researchResult2 = await tahu.runAgent(
      'MySmartResearcherJSON',
      'What was the last thing I asked you?'
    );
    console.log(
      'Researcher Agent (JSON, continued):',
      researchResult2.response
    );
    ...
    const coderResult = await tahu.runAgent(
      'MyCoderAgentSQLite',
      'Write a simple Python function to calculate factorial.'
    );
    console.log('Coder Agent (SQLite):', coderResult.response);
    // Run again to demonstrate memory persistence
    const coderResult2 = await tahu.runAgent(
      'MyCoderAgentSQLite',
      'Can you remind me about the Python function you just wrote?'
    );
    ...
    const allCapabilitiesAgent = tahu
      .builder()
      .name('OmniAgent')
      .systemPrompt(
      ...
      .addCapabilities(
        ...
        tools.summarizeTool.name, // Built-in tools
        plugins.tahuCryptoPlugin.name,
        plugins.tahuSocialPlugin.name,
        plugins.tahuFinancePlugin.name,
        plugins.tahuCurrencyPlugin.name, // Plugin tools
        tools.trainKnowledgeTool.name,
        tools.retrieveKnowledgeTool.name // New knowledge tools
      )
      .addMemory('json', {
        maxMemorySize: 10,
        memoryPath: './omni_agent_memory.json',
      })
      .build();
    ...
    const omniResult1 = await tahu.runAgent(
      'OmniAgent',
      'Find the current price of Bitcoin, then find the location of the Eiffel Tower, and provide a brief summary of its history.'
    );
    ...
    const omniResult2 = await tahu.runAgent(
      'OmniAgent',
      'What is 123 divided by 45, and what are the top social trends on TikTok?'
    );
    ...
    tahu.createAgent('WorkflowResearcher', {
      systemPrompt: 'You are a researcher who gathers information.',
    });
    tahu.createAgent('WorkflowAnalyst', {
      systemPrompt: 'You are an analyst who processes research data.',
    });
    tahu.createAgent('WorkflowWriter', {
      systemPrompt: 'You are a writer who summarizes analysis results.',
    });

    const workflow = tahu.createWorkflow([
      { agent: 'WorkflowResearcher', task: 'research_ai_trends' },
      {
        agent: 'WorkflowAnalyst',
        task: 'analyze_research',
        depends: ['research_ai_trends'],
      },
      {
        agent: 'WorkflowWriter',
        task: 'summarize_analysis',
        depends: ['analyze_research'],
      },
    ]);
    ...
    const parallelTasks = [
      ...
      { prompt: 'What is the capital of France?' },
      {
        ...
        input: 'Summarize the last research topic.',
      },
    ];
    ...
    console.log(
      `   Total Response Time: ${stats.totalResponseTimeMs.toFixed(2)} ms`
    );
    console.log(
    ...
    console.log(
      '💡 To use Supabase, you need to add Supabase integration to your project.'
    );
    console.log('\n' + '='.repeat(50) + '\n');
