...
async function ragDemo() {
    ...
    if (OPENROUTER_API_KEY.includes('XXXXXXXXXXXXX')) {
        console.error('❌ Please set your OPENROUTER_API_KEY in example/test_rag.js!');
        return;
    }
    ...
    try {
        ...
        // Train from Text
        const textData = "TahuJS adalah framework Node.js yang memudahkan pengembangan aplikasi AI. Fitur utamanya meliputi integrasi LLM, agen cerdas, dan basis pengetahuan RAG.";
        const trainTextResult = await tahu.useTool(tools.trainKnowledgeTool.name, `${knowledgeBaseName}|${storeType}|text|${textData}`);
        ...
        if (fs.existsSync(filePath)) {
            const trainFileResult = await tahu.useTool(tools.trainKnowledgeTool.name, `${knowledgeBaseName}|${storeType}|file|${filePath}`);
        ...
        const trainUrlResult = await tahu.useTool(tools.trainKnowledgeTool.name, `${knowledgeBaseName}|${storeType}|url|${urlData}`);
        ...
        if (error.message.includes('Supabase')) {
             console.error('💡 Ensure Supabase integration is configured correctly in TahuJS.');
        }
    }
}
