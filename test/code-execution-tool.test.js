...
describe('CodeExecutionTool', () => {
  ...
  test('should return correct tool information via getInfo()', () => {
    ...
    expect(info.description).toBe('Execute code safely in various programming languages');
  ...
  test('should return supported languages and examples via getSupportedLanguages()', () => {
    ...
    expect(supported.languages).toEqual(expect.arrayContaining(['javascript', 'python', 'bash', 'powershell', 'node']));
  ...
  test('should reject code with rm -rf', async () => {
    ...
    const result = await tool.execute(params);
    expect(result.success).toBe(false);
    expect(result.error).toContain('Dangerous operation detected.');
  });
  ...
  test('should reject JavaScript code with fs require', async () => {
    ...
    const result = await tool.execute(params);
    expect(result.success).toBe(false);
    expect(result.error).toContain('Restricted JavaScript operation detected.');
  });
  ...
  test('should reject Python code with os import', async () => {
    ...
    const result = await tool.execute(params);
    expect(result.success).toBe(false);
    expect(result.error).toContain('Restricted Python operation detected.');
  });
  ...
  test('should reject Bash code with sudo', async () => {
    ...
    const result = await tool.execute(params);
    expect(result.success).toBe(false);
    expect(result.error).toContain('Restricted Bash operation detected.');
  });
